...
public class WorldConfig {
    ...
    private static final String TIME_CROSS_NODE = "timeToCrossTheNode";
    ...
    public WorldConfig(Node worldConfig) {
        ...
        for (int i = 0; i < nodeConfigs.size(); ++i) {
            ...
            int timeToCrossTheNode = Integer.parseInt(nodeConfigAttrMap.get(TIME_CROSS_NODE));

            NodeConfig nodeConfigData = new NodeConfig(nodeName, capacity, timeToCrossTheNode);
            ...
            //BLOCK_TIME_TO_CROSS
            String blockTimeToCrossStr = nodeConfigAttrMap.get(BLOCK_TIME_TO_CROSS);
            if(blockTimeToCrossStr != null)
            {
                if(blockTimeToCrossStr.compareToIgnoreCase("true")==0)
                    nodeConfigData.setBlockTime(true);
            }
        ...
        for (int i = 0; i < verticeConfigs.size(); ++i) {
            ...
            VerticeConfig verticeConfigData = new VerticeConfig(source, destination, transportRate);
            this.verticeConfigs.add(verticeConfigData);
            if (!directed) {
                VerticeConfig verticeConfigDataAnotherDirection = new VerticeConfig(destination, source, transportRate);
