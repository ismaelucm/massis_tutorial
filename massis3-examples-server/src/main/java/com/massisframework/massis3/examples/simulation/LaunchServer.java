...
public class LaunchServer {
	...
	public static void main(String[] args) throws Exception {
		...
		CommandLineInfo[] clInfo = { new CommandLineInfo<String>(LancherDefs.HELP, "Help: Show the help", null),
				new CommandLineInfo<String>(LancherDefs.IP, "IP: The server IP", "<IP>",
						Configuration.instance().getHost()),
				new CommandLineInfo<Integer>(LancherDefs.PORT, "Port: The server port", "<PORT>",
						Configuration.instance().getPort()),
				new CommandLineInfo<Integer>(LancherDefs.FILE,
						"File: The json where is defined the behavior of the humans in the simulation", "<json file>"),
				new CommandLineInfo<String>(LancherDefs.SIMULATION,
						"Simulation: The predefined description of the simulation", "<scene name>", "Faculty_1floor"),
				new CommandLineInfo<String>(LancherDefs.SIMULATION_LIST, "List of predefined simulations availables",
						null) };
		CommandLineConfig config = new CommandLineConfig("Massis server, Version 3.4", clInfo, "-h");
	...
	public static void LaunchWithScene(CommandLineConfig config) throws Exception {
		PreconfiguredSimulation sim = PreconfiguredSimulation
				.getSim(config.<String>getInfo(LancherDefs.SIMULATION).getValue());
		LaunchScene(config, sim);
	}
	...
	public static void LaunchScene(CommandLineConfig config, PreconfiguredSimulation sim) throws Exception {
		...
		options.setConfig(configJson);
		vertx.deployVerticle(sim.getClassName(), options, r -> {
			if (r.failed()) {
				r.cause().printStackTrace();
				log.error("Error when launching verticle", r.cause());
			} else {
	...
	/*	String filename = "mifichero.mp4";
		...
		new Thread(){
			public void run() {
		        ...
		        es.cameraMapKeyValue(cameraId, r -> {
					if (r.failed())
					{
				      new RuntimeException("Camera "+cameraId+" not found in sim "+simId);
					} else
					{
						String localMapName = r.result().getString(0);
						String localMapKey = r.result().getString(1);
						ReadStream<Buffer> readStream = new LocalMapReadStream<String, Buffer>(vertx, 100L, localMapName, localMapKey);

						readStream.resume();
						MJPEGFileStreamer streamer = MJPEGFileStreamer.create(filename, 200,
								readStream);
						streamer.stream();
	...
	public static void Launch(CommandLineConfig config) throws Exception {
		...
		SimulationServerConfig cfg = new SimulationServerConfig()
				.withAssetFolders(Arrays.asList(Configuration.instance().getPath() + "Scenes",
						Configuration.instance().getPath() + "models",
						Configuration.instance().getPath() + "animations"))
				.withHttpServerConfig(new HttpServerConfig().withHost(config.<String>getInfo("-i").getValue())
						.withPort(config.<Integer>getInfo("-p").getValue().intValue()))
				.withAuthPropertiesFile("classpath:webauth.properties").withInstances(1)
				.withRendererType(SimulationServerConfig.RendererType.LWJGL_OPEN_GL_3)
				.withRenderMode(SimulationServerConfig.RenderMode.DESKTOP);
	...
	private static Future<Long> createSim(Vertx vertx, String sceneFile) {
		SimulationServerService proxy = Massis3ServiceUtils.createProxy(vertx, SimulationServerService.class,
				Massis3ServiceUtils.GLOBAL_SERVICE_GROUP);
		Future<Long> simCreateFuture = Future.future();
		proxy.create(sceneFile, simCreateFuture.completer());
		return simCreateFuture;
